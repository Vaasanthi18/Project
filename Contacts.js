...
const Contacts = () => {
  ...
  const getUserdata = () => {
    Axios.get("http://localhost:3002/getuserdata", {}).then((res) => {
  ...
  const submiteditdata = () => {
    if (
      ...
      Axios.post(`http://localhost:3002/submitedit/${Id}`, {
        Username: Username,
        userEmail: userEmail,
  ...
  const submitdeletedata1 = () => {
    Axios.delete("http://localhost:3002/detetedata", {data:{Id1:Id1}}).then(
      (res) => {
